...
import { useConfiguration, useColorSlice, useAromaSlice, useGeoDispatch, useZip } from "../hooks/hooks";
import { hexToRgb, rgbToString } from "../utilities/hexToRgb";
import { WeightsBarChart } from "./WeightsBarChart";

export const AromaPanel = memo(function AromaPanel() {
  const { zipName } = useZip();
    const isExpanded = useAromaSlice();
  ...
  return (
    ...
    >
      <header className="panel-header">
        {/* <h4>Metabolites</h4> */}
         {isExpanded && <h4>Related aroma atrributes</h4>}
      ...
      {isExpanded && (weightsFile && (
        weightsFile.data.aroma.length > 0 && (
        <div className="panel-content">
          ...
          <WeightsBarChart data={weightsFile.data.aroma} theme={theme} />
        </div>
        ) || (
        <div className="panel-content">
          <div className="explanation">
