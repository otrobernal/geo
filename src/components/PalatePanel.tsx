...
import { useConfiguration, useColorSlice, usePalateSlice, useGeoDispatch, useZip } from "../hooks/hooks";
import { hexToRgb, rgbToString } from "../utilities/hexToRgb";
import { WeightsBarChart } from "./WeightsBarChart";

export const PalatePanel = memo(function PalatePanel() {
  const { zipName } = useZip();
    const isExpanded = usePalateSlice();
    const dispatch = useGeoDispatch();

    if (zipName != "chemosensoryWithWeights.zip") {
      return null;
    }

  const handlePlotToggle = useCallback(
    () => dispatch({ type: "PALATE_TOGGLE" }),
    [dispatch], 
  );
  ...
  return (
    ...
    >
      <header className="panel-header">
        {/* <h4>Metabolites</h4> */}
         {isExpanded && <h4>Related palate atrributes</h4>}
      ...
      {isExpanded && (weightsFile && (
        weightsFile.data.palate.length > 0 && (
        <div className="panel-content">
          ...
          <WeightsBarChart data={weightsFile.data.palate} theme={theme} />
        </div>
        ) || (
        <div className="panel-content">
          <div className="explanation">
