...
import { useConfiguration, useColorSlice, useChemicalsSlice, useGeoDispatch, useZip } from "../hooks/hooks";
import { hexToRgb, rgbToString } from "../utilities/hexToRgb";
import { WeightsBarChart } from "./WeightsBarChart";

export const ChemicalsPanel = memo(function ChemicalsPanel() {
  const { zipName } = useZip();
    const isExpanded = useChemicalsSlice();
    const dispatch = useGeoDispatch();

    if (zipName == "allChemistryWithEmptyChemicalElement.zip") {
      return null;
    }

  const handlePlotToggle = useCallback(
    () => dispatch({ type: "CHEMICALS_TOGGLE" }),
    [dispatch], 
  );
  ...
  return (
    ...
    >
      ...
      {isExpanded && (weightsFile && (
        weightsFile.data.chemical.length > 0 && (
        <div className="panel-content">
          ...
          <WeightsBarChart data={weightsFile.data.chemical} theme={theme} />
        </div>
        ) || (
        <div className="panel-content">
          <div className="explanation">
